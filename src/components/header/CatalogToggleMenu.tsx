...
const CatalogToggleMenu: React.FC = () => {
  ...
  const [hoveredCategory, setHoveredCategory] = React.useState<number | null>(CategoryProductsData && CategoryProductsData?.length > 0 ? CategoryProductsData[0].id : null);
  ...
  return (
    <div className="toggle-catalog-menu">
      <div className="main-categories">
        {CategoryProductsData && CategoryProductsData?.length > 0 ? CategoryProductsData?.map((item: Categories) => (
               <span
      ...
      {CategoryProductsData && CategoryProductsData?.length > 0 ? CategoryProductsData?.map((item: Categories) => {
        if(hoveredCategory === item?.id) {
          return (
            item && item?.children && item?.children?.length > 0 ? item.children?.map((children: Children) => (
              <div className="links-categories" key={children?.id}>
              <Link to='/products' key={children?.id}
              onClick={() => {
              ...
              <div className="other-links">
                {hoveredCategory && hoveredCategory === item?.id && children && children?.children && children?.children?.length > 0 ? children?.children?.map((children: InnerChilds) => (
                  <Link 
                  onClick={() => {
                    //navigate products and show clicked category names
                    setClickedCategory(children?.id);
                  ...
                  to='/products' className="inner-child-link" key={children?.id}>
                    {children?.title}
                  </Link>
                )) : ""}
              </div>
